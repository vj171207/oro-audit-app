...
| Loan sync | Manual only — `api/sync-loans.js`, triggered via the "Run sync" button in Settings. **The automatic nightly Vercel Cron trigger has been removed** (as of this writing) — see "Why the cron was removed" below. |

**Data flow:** Metabase is the source of truth for live loan status and is **read-only** from this app's perspective. Firestore is the source of truth for everything the audit team enters (audit records, tare weights, settings, users). `api/sync-loans.js` pulls active loans from Metabase and adds any new ones to Firestore as pending audits — it never writes back to Metabase. This used to run automatically every night via Vercel Cron; it is now manual-only (see below) — the New Audit screen's actual loan list has never depended on this sync running, since it queries Metabase live every time it opens regardless.

### Why the cron was removed

The nightly sync wrote `metabase-sync` placeholder docs to Firestore for loans not yet audited — but every screen in the app that could have used those placeholders (New Audit's unaudited-loan list, Tare Weight, All Audits) explicitly filters them OUT instead. The actual "which loans need auditing" answer has always been computed live, by querying Metabase directly and diffing against real audit records — not from anything the cron wrote. Once this was confirmed by reading the code (not assumed), the cron was removed as dead weight with no functional loss. The manual "Run sync" button remains, unchanged, for anyone who wants to force a sync (e.g. to update the Settings "last synced" timestamp, or pre-populate placeholders for some other reason).
...
| `FIREBASE_SYNC_EMAIL` / `FIREBASE_SYNC_PASSWORD` | Credentials for a dedicated Firebase service account used by the manual sync, user creation, password reset, and user removal functions to obtain an auth token. **This is the only account permitted to write to the `users` collection** — see the Firestore Security Rules note below, this is not optional/legacy, it's load-bearing. |
| `CRON_SECRET` | Shared secret checked by `api/sync-loans.js`. Despite the name, this is no longer cron-exclusive — there is no automatic cron trigger anymore. The same secret is what the "Run sync" button in Settings sends as the entered password. Renaming this variable to something like `SYNC_SECRET` would be more accurate, but hasn't been done to avoid an unnecessary env-var rename. |
...
- `api/sync-loans.js` — Manual-trigger sync (no longer an automatic cron — see "Why the cron was removed" above): pulls active loans from Metabase, adds new ones to Firestore, updates last-sync timestamp
...
- `vercel.json` — API routing rewrites. (No longer contains a cron schedule — the automatic nightly trigger was removed; sync is manual-only now via the Settings "Run sync" button.)
