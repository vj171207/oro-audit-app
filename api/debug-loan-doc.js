...
// Returns the RAW Firestore field data (including type, e.g. stringValue vs
// timestampValue) for every audit doc matching a given loanId. This is to
// check whether loanBookingDate is genuinely a plain string, or something
// else (like a Firestore Timestamp) that LOOKS fine when displayed via
// formatDate() but sorts incorrectly with localeCompare().
//
// GET /api/debug-loan-doc?loanId=TCGL31572175&secret=<BACKFILL_SECRET>
      ...
      (data.documents || []).forEach(doc => {
        if (doc.fields?.loanId?.stringValue === loanId) {
          matches.push({
            ...
            allFields: doc.fields // raw Firestore field wrapper — shows the actual type key (stringValue, timestampValue, nullValue, etc.)
          });
        }
      });
