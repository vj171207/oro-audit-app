...
function autoCalcNWi(i) {
  ...
  nwEl.value = Math.max(0, gw - (stone * (karat / 22))).toFixed(2);
}
...
function renderTWTable(search = '', filter = twFilter) {
  ...
  if (!filtered.length) {
    tbody.innerHTML = `<tr><td colspan="9" style="text-align:center; padding:40px 20px; color:var(--text-3); font-size:13px;">${loans.length === 0 ? '📋 No tare weight records yet — audits with tare weight will appear here.' : '🔍 No loans match the current filter. Try adjusting the branch or status filter.'}</td></tr>`;
...
function updateTWRowDisplay(loanId) {
  ...
  if (cells[6]) cells[6].innerHTML = diffDisplay;
  if (cells[7]) cells[7].innerHTML = badge;
