...
function renderTWTable(search = '', filter = twFilter) {
  ...
  const loans = Object.values(loanMap).sort((a, b) => {
    ...
    // actually entered Oro's system), most recent first. This is deliberately
    // different from audit date (when the team happened to audit it) per
    // Rijin's request. Loans with no booking date on record (rare, mostly
    // historical) sort to the very bottom of this group rather than the top.
    if (!a.loanBookingDate && !b.loanBookingDate) return 0;
    if (!a.loanBookingDate) return 1;
    if (!b.loanBookingDate) return -1;
    return (b.loanBookingDate || '').localeCompare(a.loanBookingDate || '');
  });
